<!-- Logo & Title -->

<h1 align="center">

<br> Occupancy models in R

</h1>

<!-- View presentation -->

<br>

<p align="center">

<a href="https://lisanicvert.github.io/2026-01-06-occupancy-models-in-r" target="_blank"><b>View
presentation</b></a>

</p>

<br>

<!-- Badges -->

<p align="center">

<!-- Quarto --> <a href="https://quarto.org/">
<img src="https://img.shields.io/badge/Made%20with-Quarto-blue.svg" alt="Quarto"/>
</a>

<!-- License -->
<a href="https://choosealicense.com/licenses/cc-by-4.0/">
<img src="https://img.shields.io/badge/License-CC%20BY%204.0-green.svg" alt="License CC BY 4.0"/>
</a>

<br/>

<!-- Quarto render -->
<a href="https://github.com/LisaNicvert/2026-01-06-occupancy-models-in-r/actions/workflows/quarto-render.yml">
<img src="https://github.com/LisaNicvert/2026-01-06-occupancy-models-in-r/actions/workflows/quarto-render.yml/badge.svg" alt="GHA render"/>
</a>

<!-- GitHub deployment -->
<a href="https://github.com/LisaNicvert/2026-01-06-occupancy-models-in-r/actions/workflows/pages/pages-build-deployment">
<img src="https://github.com/LisaNicvert/2026-01-06-occupancy-models-in-r/actions/workflows/pages/pages-build-deployment/badge.svg" alt="GHA deploy"/>
</a>

</p>

## Overview

This repository contains a presentation on occupancy models in R. 
The associated blog post is available on [FRB-CESAB's blog](https://frbcesab.github.io/tips-and-tricks/posts/2026-01-06-occupancy-models-in-r/).

## Citation

Nicvert L (2026) *Occupancy models in R*. Online presentation available
at <https://lisanicvert.github.io/2026-01-06-occupancy-models-in-r>.

## Acknowledgments

This presentation has been prepared for FRB-CESAB's geek meeting. Thanks
to [Nicolas Casajus](https://nicolascasajus.fr/) for providing this 
[template](https://github.com/biodiversitydata/quarto-template) for the slides, 
and for his help with integrating the blog post on the 
[Tips and tricks](https://frbcesab.github.io/tips-and-tricks/) website!
